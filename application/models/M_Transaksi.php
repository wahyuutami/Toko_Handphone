...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_hp', $this->input->post('kode_hp')[$i])->get('hp')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_hp', $this->input->post('kode_hp')[$i])->update('hp', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_hp'=>$this->input->post('kode_hp')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('hp', 'hp.kode_hp = detail_transaksi.kode_hp')
